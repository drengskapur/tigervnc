#!/bin/bash
# Run all TigerVNC tests and generate a report
...
REPORT_FILE="$REPORT_DIR/tigervnc_test_report_$TIMESTAMP.md"
...
# Initialize report
echo "# TigerVNC Test Report - $(date)" > "$REPORT_FILE"
echo "" >> "$REPORT_FILE"
...
echo "" >> "$REPORT_FILE"
echo "### TigerVNC Container" >> "$REPORT_FILE"
echo '```' >> "$REPORT_FILE"
docker inspect tigervnc | grep -E 'Image|Status|Health|Port' >> "$REPORT_FILE"
echo '```' >> "$REPORT_FILE"
echo "" >> "$REPORT_FILE"

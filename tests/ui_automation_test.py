...
def run_vnc_ui_test():
    """Run automated UI tests on TigerVNC connection"""
    # Start VNC server
    server_cmd = ["tigervncserver", ":1", "-geometry", "1024x768"]
    ...
    try:
        ...
        pyautogui.typewrite('echo "TigerVNC UI Test Successful" > /tmp/tigervnc_ui_test.log\n')
        time.sleep(2)
        ...
        check_cmd = ["DISPLAY=:1", "cat", "/tmp/tigervnc_ui_test.log"]
        result = subprocess.run(" ".join(check_cmd), shell=True, capture_output=True, text=True)
        
        success = "TigerVNC UI Test Successful" in result.stdout

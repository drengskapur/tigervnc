...
def measure_latency(operations=10):
    """Measure TigerVNC latency by performing X11 operations"""
    results = []
    
    # Start VNC server
    subprocess.run(["tigervncserver", ":1", "-geometry", "1024x768"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TigerVNC performance test")
    parser.add_argument("--operations", type=int, default=10, help="Number of operations to perform")
    args = parser.parse_args()
    
    print(f"Testing TigerVNC performance...")
